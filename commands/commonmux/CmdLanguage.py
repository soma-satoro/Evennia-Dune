...
class CmdLanguage(MuxCommand):
    ...
    def func(self):
        ...
        if "native" in self.switches:
            ...
            old_native = self.caller.db.native_language or "The Truth"
            self.caller.db.native_language = proper_language
            
            # Get current languages and ensure both The Truth and the native language are included
            languages = self.caller.get_languages()
            
            # Remove the old native language if it's not The Truth
            if old_native != "The Truth" and old_native in languages:
                languages.remove(old_native)
            
            # Ensure The Truth and new native language are included
            if "The Truth" not in languages:
                languages.append("The Truth")
            ...
            if old_native != "The Truth" and old_native != proper_language:
    ...
    def list_languages(self):
        ...
        native_language = self.caller.db.native_language or "The Truth"  # Default to The Truth if not set
        ...
        if language_merit_points > 0:
            # Calculate used languages, excluding The Truth and native language
            used_languages = len(languages)
            if "The Truth" in languages:
                used_languages -= 1  # The Truth is always free
            if native_language in languages and native_language != "The Truth":
    ...
    def add_language(self):
        ...
        # Get native language
        native_language = self.caller.db.native_language or "The Truth"
    ...
    def do_set_languages(self):
        ...
        for lang in self.rhs.split(','):
            ...
            if not lang or lang.lower() == "the truth" or lang.lower() == "truth":  # Skip empty or The Truth
                continue
    ...
    def list_all_languages(self):
        ...
        # Define categories and their languages for Numenera
        categories = {
            "Most Common": [
                "The Truth"
            ],
            "Steadfast - River Languages": [
                "Sadarac", "Wyrac", "Tithac"
            ],
            "Steadfast - Coastal Languages": [
                "Coras", "Coras (Northern)", "Coras (Central)", "Coras (South)"
            ],
            "The Beyond - Major Languages": [
                "Mathan", "Maran", "Seshan", "Salter", "Ba-Du"
            ],
            "The Black Riage": [
                "Black Riage (North)", "Black Riage (Central)", "Black Riage (South)",
                "Black Riage (East)", "Black Riage (West)"
            ],
            "Plains and Jungles": [
                "Kataru", "Kataru (Nomadic)", "Caecilian"
            ],
            "Beyond the Beyond": [
                "Augurian"
            ],
            "Trade Languages": [
                "Trader's Cant", "Traveler's Tongue"
            ]
        }
    ...
    def remove_language(self):
        ...
        # Can't remove The Truth
        if language.lower() == "the truth" or language.lower() == "truth":
            self.caller.msg("You cannot remove The Truth.")
            return
        ...
        for lang_key, proper_lang in AVAILABLE_LANGUAGES.items():
            if lang_key.lower() == language.lower():
                if proper_lang in current_languages:
                    ...
                    # If they were speaking the removed language, reset to The Truth
                    if target.get_speaking_language() == proper_lang:
                        target.db.speaking_language = "The Truth"
                        target.msg(f"Your speaking language has been reset to The Truth.")
    ...
    def view_character_languages(self):
        ...
        language_merit_points = 0
        native_language = target.db.native_language or "The Truth"  # Default to The Truth if not set
    ...
    def validate_languages(self, caller=None):
        ...
        languages = target.get_languages()
        native_language = target.db.native_language or "The Truth"
        ...
        # Always keep The Truth and native language
        allowed_languages = ["The Truth"]
        if native_language != "The Truth":
