# Roster Characters - Quick Copy-Paste Setup

This file contains all commands for setting up roster characters, organized for easy copy-paste.

**IMPORTANT:** Puppet each character with `@ic <character name>` before pasting their command block!

---

## Table of Contents

1. [Alia Atreides](#1-alia-atreides---regent-of-the-imperium)
2. [Leto II Atreides](#2-leto-ii-atreides---heir-to-the-empire)
3. [Ghanima Atreides](#3-ghanima-atreides---the-wise-twin)
4. [Duncan Idaho](#4-duncan-idaho---ghola-swordmaster)
5. [Irulan Corrino](#5-irulan-corrino---empress-and-historian)
6. [Lady Jessica](#6-lady-jessica---bene-gesserit-reverend-mother)
7. [Stilgar](#7-stilgar---fremen-naib)
8. [Gurney Halleck](#8-gurney-halleck---warmaster)
...
1. **Puppet the character:**
   ```
   @ic <Character Name>
   ```

2. **Copy and paste the entire command block** for that character (below)

3. **Exit back to your character:**
   ```
   @ooc
   ```
...
## 2. LETO II ATREIDES - Heir to the Empire
...
## 3. GHANIMA ATREIDES - The Wise Twin
...
## 4. DUNCAN IDAHO - Ghola Swordmaster
...
After setting up each character, verify:
...
Expected results:
- ✅ Skills set correctly (sum should match character level)
- ✅ 4-8 Focuses listed
- ✅ 5-7 Talents listed
- ✅ Drives rated 8, 7, 6, 5, 4 (one of each)
- ✅ Drive statements for drives rated 6+
- ✅ Biography sections filled in
...
- Make sure you're puppeting the character first (`@ic <name>`)
- Check you have proper permissions (Builder+)

**Biography text too long?**
- The `+bio` command has character limits
- Break very long text into shorter segments

**Can't add language?**
- Check the language exists in your game
- Use `+language/list` to see available languages

**Drives not saving?**
- Make sure ratings are 8, 7, 6, 5, 4 (one of each)
- Drive statements required for 6+

---

*All commands ready to copy-paste! Just puppet each character and paste their block.*
